...
public class EntityItachi extends ElementsNarutomodMod.ModElement {
	...
	public void initElements() {
		elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("narutomod", "itachi"), ENTITYID)
				.name("itachi").tracker(64, 3, true).egg(-16777216, -65485).build());
		// elements.entities.add(() ->
		// EntityEntryBuilder.create().entity(EntityArrowCustom.class)
		// .id(new ResourceLocation("narutomod", "entitybulletitachi"),
		// ENTITYID_RANGED).name("entitybulletitachi").tracker(64, 1, true)
		// .build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		Biome[] spawnBiomes = {Biomes.FOREST, Biomes.TAIGA, Biomes.SWAMPLAND, Biomes.RIVER, Biomes.FOREST_HILLS, Biomes.TAIGA_HILLS, Biomes.JUNGLE,
				Biomes.JUNGLE_HILLS, Biomes.BIRCH_FOREST, Biomes.BIRCH_FOREST_HILLS, Biomes.ROOFED_FOREST, Biomes.SAVANNA, Biomes.EXTREME_HILLS};
	...
	public void preInit(FMLPreInitializationEvent event) {
		 ...
		 * renderManager -> { return new
		 * RenderSnowball<EntityArrowCustom>(renderManager, null,
		 * Minecraft.getMinecraft().getRenderItem()) { public ItemStack
		 * getStackToRender(EntityArrowCustom entity) { return new
		 * ItemStack(ItemShuriken.block, (int) (1)); } }; });
		 */
	}
	public static class EntityCustom extends EntityNinjaMob.Base implements IMob, IRangedAttackMob {
		...
		protected void initEntityAI() {
			...
			this.tasks.addTask(1, new EntityNinjaMob.AIAttackRangedTactical(this, 1.0D, 50, 16.0F) {
				...
				public boolean shouldExecute() {
					return super.shouldExecute() && EntityCustom.this.getAttackTarget()
							.getDistance(EntityCustom.this) > (EntityCustom.this.isSusanooActive()
									? ProcedureUtils.getReachDistance(EntityCustom.this.susanooEntity)
									: 4d);
				}
			});
			...
			this.tasks.addTask(3, new EntityAIAttackMelee(this, 1.0d, true) {
				...
				protected double getAttackReachSqr(EntityLivingBase attackTarget) {
					return EntityCustom.this.isSusanooActive()
							? ProcedureUtils.getReachDistanceSq(EntityCustom.this.susanooEntity)
			...
			this.targetTasks.addTask(2, new EntityAINearestAttackableTarget(this, EntityPlayer.class, 10, true, false, new Predicate<EntityPlayer>() {
				public boolean apply(@Nullable EntityPlayer p_apply_1_) {
					return p_apply_1_ != null && (ItemSharingan.wearingAny(p_apply_1_) || EntityBijuManager.isJinchuriki(p_apply_1_));
				}
			}));
		}
		...
		public SoundEvent getDeathSound() {
			return (SoundEvent) SoundEvent.REGISTRY.getObject(new ResourceLocation("entity.illusion_illager.death"));
		}
		...
		public boolean attackEntityFrom(DamageSource source, float amount) {
			...
			if (!this.world.isRemote) {
				...
				if (this.rand.nextInt(3) <= 1) {
					this.setPositionAndUpdate(this.posX + (this.rand.nextDouble() - 0.5) * 2, this.posY,
							this.posZ + (this.rand.nextDouble() - 0.5) * 2);
					if (entity1 instanceof EntityLivingBase) {
						this.setRevengeTarget((EntityLivingBase) entity1);
					}
					return false;
				} else if (this.isReal && this.getHealth() > 0 && this.getHealth() - amount <= this.getMaxHealth() / 3 && !this.isRiding()
						&& this.ticksExisted > this.lastSusanooTime + 600 && this.getChakra() >= SUSANOO_CHAKRA) {
		...
		public boolean getCanSpawnHere() {
			return super.getCanSpawnHere() && this.world.getEntitiesWithinAABB(EntityCustom.class, this.getEntityBoundingBox().grow(128.0D)).isEmpty()
		...
		public void attackEntityWithRangedAttack(EntityLivingBase target, float distanceFactor) {
			...
			if (this.getRidingEntity() instanceof EntitySusanooClothed.EntityCustom) {
				...
				if (chance == 0 && distanceFactor > 0.3333f && this.consumeChakra(AMATERASU_CHAKRA)) {
					this.world.playSound(null, target.posX, target.posY, target.posZ,
							(SoundEvent) SoundEvent.REGISTRY.getObject(new ResourceLocation("narutomod:sharingansfx")), SoundCategory.NEUTRAL, 1f,
							1f);
	...
	public class RenderCustom extends EntityNinjaMob.RenderBase<EntityCustom> {
		...
		public RenderCustom(RenderManager renderManagerIn, ModelBiped modelIn) {
			...
			// this.addLayer(new
			// net.minecraft.client.renderer.entity.layers.LayerCustomHead(modelIn.bipedHead));
			// this.addLayer(new
			// net.minecraft.client.renderer.entity.layers.LayerHeldItem(this));
