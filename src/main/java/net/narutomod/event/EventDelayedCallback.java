...
public class EventDelayedCallback extends SpecialEvent {
	...
	public EventDelayedCallback(World worldIn, int x, int y, int z, long startTime, Callback callbackIn) {
		super(EnumEventType.DELAYED_CALLBACK, worldIn, null, x, y, z, startTime, false, false);
		if (!worldIn.isRemote) {
	...
	protected void onUpdate() {
		...
		super.onUpdate();
		this.callback.execute(this.world, this.x0, this.y0, this.z0);
		this.clear();
	}
	...
	public static abstract class Callback {
		...
		public abstract void execute(World world, int x, int y, int z);
	}
}
